...
(function(){
	cpr.core.Module.define("module/standard/common", function(exports, globals, module){
		...
		AppKit.prototype.validate = function(app, paCtlId, psDataScope, isMsgAlert) {
			...
			for(var i=0, len=paCtlId.length; i<len; i++) {
				...
				if(ctrl instanceof cpr.controls.Grid){ // 그리드 컨트롤
					...
					var vbIsHideOnGrid = ctrl.userAttr("_hideOnGrid_") == "true" && ctrl.userAttr("_originVisible_") == "true";
					if(!vbIsHideOnGrid) {
		 ...
		 * @param {Boolean} isMsgAlert validation 메시지 출력 여부
		 * @param {"all" | "modify"} dataScope 그리드 내 컨트롤 유효성 검사 시, 그리드 유효성 검증으로 전환
		...
		AppKit.prototype._validateFreeForm = function(poForm, isMsgAlert) {
			...
			var vbIsHideOnGrid = form.userAttr("_hideOnGrid_") == "true" && form.userAttr("_originVisible_") == "true";
			...
			var allTargetControls = allChildControls.filter(function(ctrl){
				...
				if(vbIsHideOnGrid) {
					...
					if (ctrl.visible == false && ctrl.userAttr("mustValidate") != "Y") return false;
				} else {
					
					if (ctrl.isShowing() == false && ctrl.userAttr("mustValidate") != "Y") return false;
				}
		...
		AppKit.prototype._validateGridFromRowIdex = function(poGrid , paRowIndexs , isMsgAlert) {
			...
			if(!ValueUtil.isNull(vsDataBindCtxId)){
				...
				vbValidatableOnGrid = grd.detail.getCellIndices().some(function(each){
					var vcCtrl = grd.detail.getControl(each);
					if(vcCtrl) {
				...
				for(var i = 0; i < cellCnt; i++) {
					...
					if(ValueUtil.isNull(vsDataBindCtxId) || vbValidatableOnGrid) {
						...
						if(vbValidateMust || vbValidatableOnGrid === false) {
							...
							grd.selectRows([idx]);
							var bools = _this._validateFreeForm(grd.getAppInstance().lookup(vsDataBindCtxId),isMsgAlert,true);
							return !bools;
						}
					}
					// 컨트롤별 Validation Check
...
(function(){
	cpr.core.Module.define("module/standard/expression", function(exports, globals, module){
		 ...
		 * @param {Number} value 반올림할 숫자. 유효한 숫자가 아니거나 무한대(Infinity)인 경우 0을 반환.
		 * @param {Number} position? 반올림할 소수점 이하 자릿수. 지정하지 않으면 정수 단위로 반올림.
		 ...
		 * @param {Number} value 내림 처리할 숫자. 유효한 숫자가 아니거나 무한대(Infinity)인 경우 0을 반환.
		 * @param {Number} position? 내림 처리할 소수점 이하 자릿수. 지정하지 않으면 정수 단위로 내림 처리.
		 ...
		 * @param {Number} value 포맷팅할 숫자. 숫자가 아닌 경우 원본 값이 그대로 반환.
...
(function(){
	cpr.core.Module.define("module/standard/extension", function(exports, globals, module){
		...
		*  @return {String} Row을 찾을 수 없을 경우 null을 반환하고 그렇지 않은 경우 아래 값 중 하나를 반환합니다.<br>
		...
		FreeFormKit.prototype.setRowVisible = function(app, paFreeFormId, paIndex, pbVisible) {
			...
			for (var i = 0, len = paFreeFormId.length; i < len; i++) {
				...
				for (var j = 0, len = paIndex.length; j < len; j++){	
					if(voRForm){
						if((voRForm._screenNms["tablet"]).indexOf(screenNm) > -1){
						...
						} else if((voRForm._screenNms["mobile"]).indexOf(screenNm) > -1){
			...
			psGridId.forEach(function(gridId, pnIndex) {
				...
				var styleHandler = function(cellStyle, region, cellIndex, columnName) {
					...
					if (options.applyClassStyle) {
						...
						// 해당 컬럼의 클래스
						if (region == "header") {
							if (vcGrid.header.getControl(cellIndex)) {
								vaClassName = vcGrid.header.getControl(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컨트롤 클래스 명
							}
							if (vcGrid.header.getColumn(cellIndex)) {
								vaClassName = vcGrid.header.getColumn(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컬럼 클래스 명
							}	
						}else if(region == "detail"){
							if (vcGrid.detail.getControl(cellIndex)) {
								vaClassName = vcGrid.detail.getControl(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컨트롤 클래스 명
							}
							if (vcGrid.detail.getColumn(cellIndex)) {
								vaClassName = vcGrid.detail.getColumn(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컬럼 클래스 명
							}	
						}else if(region == "footer"){
							if (vcGrid.footer.getControl(cellIndex)) {
								vaClassName = vcGrid.footer.getControl(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컨트롤 클래스 명
							}
							if (vcGrid.footer.getColumn(cellIndex)) {
								vaClassName = vcGrid.footer.getColumn(cellIndex).style.getClasses();
								vaClass = vaClass.concat(vaClassName instanceof Array ? vaClassName : [vaClassName]); // 컬럼 클래스 명
							}
						}
					    ...
					    vaClass.forEach(function (each) {
					        styles.forEach(function (style) {
					            var cssValue = _this._getComputedCss(each, style);
					            if (cssValue) {
					                voStyleInfo[style] = cssValue;
					            }
					        });
					    });
					}
					...
					// 인라인 스타일이 우선 순위
					voStyleInfo = Object.assign({}, voStyleInfo, cellStyle["style"]);
		 ...
		 */
		GridKit.prototype._fillZero = function(width, str) {
		...
		GridKit.prototype._OWNER_SET = new Set();
		 ...
		 */
		GridKit.prototype._getStyleSheetArray = function(classArr) {
			var _this = this;
			...
			if (_this._OWNER_SET.has(JSON.stringify(vaClass))) {
				return;
			}
			_this._OWNER_SET.add(JSON.stringify(vaClass));
			var results = {};
					...
					rules.forEach(function(/*String*/eachRule){
						...
						if(rules.length > 1){
							if(eachRule.indexOf(".cl-grid") != -1){
								...
								var returns = _this._arrayToObject(classInfo);
								_this._assign(_this._CLASS_INFO, returns);
							}
						}else{
							if(eachRule.indexOf(eachClass+",") != -1) {
								eachRule =  eachRule.replace(eachRule.slice(eachRule.indexOf(","),eachRule.indexOf("{")-1),"");
		 ...
		 */
		GridKit.prototype._getStyleSheet = function(className) {
			var _this = this;
			if (_this._CLASS_INFO.hasOwnProperty(className)) {
				return null;
			}
			var vaRules = [];
		 ...
		 * @param {Array} pArray
		 */
		GridKit.prototype._arrayToObject = function(pArray) {
			var _this = this;
			const value = pArray.pop(); // 배열의 마지막 값을 추출합니다.
			var nestedObject = _this._cssStringToObject(value);
		 ...
		 */
		GridKit.prototype._cssStringToObject = function(cssString) {
			var _this = this;
			var cssObject = {};
		 ...
		 * @param {Object} arg2
		 */
		GridKit.prototype._assign = function(arg1, arg2) {
			var _this = this;
			...
			Object.keys(arg2).forEach(function(each) {
				...
				if (arg1.hasOwnProperty(each)) {
					...
					_this._assign(arg1[each], arg2[each]);
				} else {
					arg1[each] = arg2[each];
				}
			});
		 ...
		 */
		GridKit.prototype._getComputedCss = function(className, propertyNm) {
			var _this = this;
			if (!_this._CLASS_INFO.hasOwnProperty("." + className)) {
				return null;
			}
			var vsPropertyName = propertyNm;
			var voClass = _this._CLASS_INFO["." + className];
			...
			var vsBorderSideNm = propertyNm.replace(/(border-\w+)-(color|style)/g, "$1"); 						// border-(left|right|top|bottom)
			...
			if (target.indexOf("rgb") != -1 && !/(border-)(\w*-)(style)/g.test(propertyNm) && !/(border-)(\w*-)(width)/g.test(propertyNm)) {
				...
				if (background && background.length > 0) {
					var newBgColor = _this._fillZero(2, parseInt(background[0]).toString(16).toUpperCase()) +
						_this._fillZero(2, parseInt(background[1]).toString(16).toUpperCase()) +
						_this._fillZero(2, parseInt(background[2]).toString(16).toUpperCase());
